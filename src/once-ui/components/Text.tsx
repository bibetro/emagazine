...
const Text = <T extends ElementType = "span">({
  ...
  const getVariantClasses = (variant: TextVariant) => {
    // Handle simple variants like 'display', 'overline'
    if (!variant.includes('-')) {
      return [`font-${variant}`];
    }
    
    // Handle two-part variants like 'title-l', 'body-m'
    const parts = variant.split('-');
    if (parts.length === 2) {
      const [fontType, size] = parts;
      return [`font-${fontType}`, `font-${size}`];
    }
    
    // Handle three-part variants like 'body-default-m'
    const [fontType, weight, size] = parts;
    return [`font-${fontType}`, `font-${weight}`, `font-${size}`];
  };

  const sizeClass = size ? `font-${size}` : "";
  let weightClass = weight ? `font-${weight}` : "";
  
  // Special handling for common weight values
  if (weight === "bold") {
    weightClass = "font-strong";
  }
